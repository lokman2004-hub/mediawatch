# =============================================================================
# SCRAPER AL JAZEERA — Projet : architecture
# =============================================================================
import requests
...
class AlJazeeraScraper:
    def __init__(self):
        ...
        logger.info("AlJazeeraScraper initialisé")
    ...
    def get_links(self, section_url, category):
        ...
        for a in soup.find_all("a", href=True):
            ...
            if ("aljazeera.com" in href and "/20" in href
                    and "video" not in href and "liveblog" not in href
                    and href.count("/") >= 4):
                if href not in links:
                    links.append(href)
        links = links[:self.max_per_section]
        logger.info(f"[AlJazeera/{category}] {len(links)} liens trouvés")
        return links
    ...
    def scrape_article(self, url, category):
        ...
        if not titre:
            t = soup.find("title")
            if t:
                titre = t.get_text(strip=True).replace(" | Al Jazeera", "")
    ...
    def run(self):
        logger.info("=" * 55)
        logger.info("SCRAPER AL JAZEERA")
        logger.info("=" * 55)
        valides = []
        ...
        logger.info(f"Al Jazeera — {len(valides)} articles valides")
        return valides

if __name__ == "__main__":
    articles = AlJazeeraScraper().run()
